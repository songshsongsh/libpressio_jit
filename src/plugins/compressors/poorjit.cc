...
public:
  ...
  struct pressio_options get_configuration_impl() const override
  {
    ...
    set(options, "predictors:error_dependent",
        get_accumulate_configuration("predictors:error_dependent", invalidation_children, invalidations));
    set(options, "predictors:error_agnostic",
        get_accumulate_configuration("predictors:error_agnostic", invalidation_children, invalidations));
  ...
  struct pressio_options get_documentation_impl() const override
  {
    ...
    set_meta_docs(options, "poorjit:generator", "generator used to produce the source code", gen_impl);
  ...
  int set_options_impl(struct pressio_options const& options) override
  {
    ...
    // 2. send options to generator
    //    let pycuke generator have pycuke:frontend_code etc.
    int rc = gen_impl->set_options(options);
    if (rc != 0) {
        return set_error(gen_impl->error_code(), gen_impl->error_msg());
    }

    // 3. handle JIT parameters
  ...
  int compress_many_impl(compat::span<const pressio_data* const> const& inputs,
  ...
  int decompress_many_impl(compat::span<const pressio_data* const> const& inputs,
                           compat::span<pressio_data*>& outputs) override {
