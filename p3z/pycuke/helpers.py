...
def is_scalar(v):
    return isinstance(v, int) or isinstance(v, float) or (isinstance(v, asg.Tensor) and len(v.ref_size) == 0)
            ...
            if lhs != None:
                ...
                if rhs != None:
                    if e.op_type == 'add':
                        ...
                        return lhs / rhs
                    elif e.op_type == 'shift_left':
                        return lhs << rhs
                    elif e.op_type == 'shift_right':
                        return lhs >> rhs
                    elif e.op_type == 'bitwise_and':
                        return lhs & rhs
                    elif e.op_type == 'bitwise_or':
                        return lhs | rhs
                    elif e.op_type == 'bitwise_xor':
                        return lhs ^ rhs
                    else:
                        return None
                else:
...
def broadcastable(shape1, shape2):
    # Check if shape2 can broadcast to shape1 (elementwise op)
    s1 = list(shape1)[::-1]
    s2 = list(shape2)[::-1]
    for i in range(len(s2)):
        if not (s1[i].val == s2[i].val or s2[i].val == 1):
            return False
    return True
...
def autotune(dataset, block_size: int,
                pe_rows_max: int, pe_cols_max: int):

    # calculate total block number
    with open(dataset, 'rb') as file:
        raw_data = file.read()
    raw_data = struct.unpack('f' * (len(raw_data) // 4), raw_data)
    N = len(raw_data)
    block_num = (N + block_size - 1) // block_size

    # get the config with larger P, more square PEs, and higher row count
    P_target = min(block_num, pe_rows_max * pe_cols_max)
    best = None
    best_Pr, best_Pc = 1, 1
    for Pr in range(1, pe_rows_max + 1):
        for Pc in range(1, pe_cols_max + 1):
            P = Pr * Pc
            if P <= P_target:
                key = (-P, abs(Pr - Pc), -Pr)
    ...
    P_used = best_Pr * best_Pc
    blocks_per_pe = (block_num + P_used - 1) // P_used

    return (blocks_per_pe, best_Pr, best_Pc)

def get_num_blocks(path: str, block_size: int):
