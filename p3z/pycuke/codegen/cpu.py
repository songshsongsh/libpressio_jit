...
def to_string(stmt):
    if isinstance(stmt, ir.Expr):
            ...
            elif stmt.op == 'mask_bigger':
                return f"({to_string(stmt.left)} > {to_string(stmt.right)})"
            elif stmt.op == 'smaller':
                return f"({to_string(stmt.left)} < {to_string(stmt.right)} ? ({to_string(stmt.left)}) : ({to_string(stmt.right)}))"
            elif stmt.op == 'mask_smaller':
                return f"({to_string(stmt.left)} < {to_string(stmt.right)})"
    elif isinstance(stmt, ir.IfExpr):
            return f"({to_string(stmt.cond)} ? {to_string(stmt.true_val)} : {to_string(stmt.false_val)})"
    elif isinstance(stmt, ir.IfElse):
        cond = to_string(stmt.cond)
        then_body = to_string(stmt.then_stmt)
        else_body = to_string(stmt.else_stmt)
        return f"if ({cond}) {{\n{then_body}}}\nelse {{\n{else_body}}}\n"
    ...
    elif isinstance(stmt, ir.Cast):
        to_t = stmt.to_dtype
        if to_t not in type_map:
            raise KeyError(f'Unsupported cast dtype: {to_t}')
        return f"static_cast<{to_t}>({to_string(stmt.val)})"
