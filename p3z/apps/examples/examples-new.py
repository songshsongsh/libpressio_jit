...
from pycuke.asg import Tensor, Var, Const, apply, bigger, einsum, inline, mk_list, cons, car, cdr, new_op
...
def test1():
    ...
    B = Tensor((10, 20))
    res = A + B - 1
    code = codegen.cpu_pytorch.print_cpp(gen_ir(res))
    print(code)
...
def test3():
    ...
    code = codegen.cpu_pytorch.print_cpp(gen_ir(ast))
    print(code)
...
def test10():
    ...
    code = codegen.cpu_pytorch.print_cpp(ir)
    print(code)
...
def apply_test8():
    ...
    code = codegen.cpu_pytorch.print_cpp(res._gen_ir())
    print(code)
...
def test_apply5():
    ...
    code = codegen.cpu_pytorch.print_cpp(v0_nb._gen_ir())
    print(code)
...
def prefix_sum1():
    ...
    code = codegen.cpu_pytorch.print_cpp(psum._gen_ir())
    print(code)
...
def view_test12():
    data = Tensor((4, 8, 16))
    view1 = data.view((64, 8), ([0, 2], 1))
    view2 = view1.view((512, ), ([0, 1], ))
    i = Var(name='i')
    tmp1 = view2[i]
    tmp2 = Var(name='tmp2')
    res = tmp1 + tmp2
...
def view_test13():
    data = Tensor((1024, ))
    view1 = data.view((16, 64), (0, 0))
    view2 = view1.view((64, 16, 16), (1, -1, 0))
    tmp2 = Tensor(view2.ref_size)
    res = view2 + tmp2
    code = codegen.cpu_pytorch.print_cpp(gen_ir(res))
    print(code)
...
def neg_transE():
    ...
    nblocks = Const(batchsize / blocksize, dtype='int')
...
def neg_transR():
    ...
    blocksize = Var(name='blocksize')
    nblocks = Const(batchsize / blocksize, dtype='int', name='nblocks')
    output_size = Const(batchsize * blocksize, dtype='int', name='output_size')
...
def intersect():
    ...
    src = '''
    _intersect(A, B, C, POS);
    '''

    t = inline(src, [('C', C), ('pos', pos)], [('A', A), ('B', B)])

    pos = t.at(1)

    res = t.at(0)[:pos] + A[:pos]
    code = codegen.cpu_pytorch.print_cpp(gen_ir(res))
    print(code)
...
def test_list1():
    ...
    t = mk_list(z, mk_list(mk_list(x, a), y), b)

    res = t.at(1).at(0).at(1) + t.at(1).at(1) + t.at(2)
...
def test_func1():
    ...
    res = mk_list(f1(Const(0), Const(1)), g(Const(0), Const(1)))
    code = codegen.cpu_pytorch.print_cpp(gen_ir(res))
    print(code)
...
# compiling a functional program means it is evaluated in normal-order
def test_func2():
    @new_op
    def square(x):
        return x * x
...
# but we can always store intermediate results to avoid redundant computation.
def test_func3():
    @new_op
    def square(x):
        return x * x
