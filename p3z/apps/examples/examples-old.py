...
import pycuke1.helpers as helpers
import pycuke1.transform
from pycuke1.asg import Tensor, Var, apply, bigger, setval, einsum
from pycuke1.asg2ir import gen_ir
import pycuke1.codegen as codegen
import pycuke1.run
...
def test_math2():
    input = Tensor((50, 32), dtype='float')
...
N = A[1:99]
L = 2*A[2:99] - A[1:98]
Q = 3*A[3:99] - 3*A[2:98] +A[1:97]

python new tensor O = [1,2,3]

cond = bigger(bigger(N, L), Q)
Output = apply(setval(A, ?), cond=cond)
python pybind
# ipdps
def conv1d_v1():
    A = Tensor((100, ))
